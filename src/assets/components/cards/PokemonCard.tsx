...
const PokemonCard: React.FC<PokemonDetails> = ({
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.9 }}
      className="w-full h-40 p-3 rounded-lg flex flex-col justify-center items-start hover:cursor-pointer hover:shadow-lg "
          ...
          <ul className="text-sm ml-1">
            ...
            <li className="w-40 text-nowrap overflow-hidden text-ellipsis">
              ...
              {abilities.map((ability, index) => (
                <span key={index}>
                  {index > 0 && ", "}{" "}
                  {/* Añade una coma y un espacio si no es el primer elemento */}
